...
function Login(props) {
    ...
    const submitForm = async (event) => {
        ...
        if (data.success && data.token) {
            ...
            await isLoggedIn();
            await props.history.push('/services');
        }
    ...
    let errorMsg = '';
    if (state.errorMsg) {
        errorMsg = <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
            <strong class="font-bold">⚠  </strong>
            <span class="block sm:inline">{state.errorMsg}</span>
           
        </div>;
    }
    if (state.successMsg) {
        successMsg = <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
            <strong class="font-bold">✓  </strong>
            <span class="block sm:inline">{state.successMsg}</span>
                ...
                <div className="py-12 bg-indigo-100 lg:bg-white flex justify-center lg:justify-start lg:px-12">
                    <div className="cursor-pointer flex items-center">
                        <img
                            ...
                            alt="FindNearby"
                        />
                        <div class="text-2xl text-indigo-800 tracking-wide ml-2 font-semibold">FindNearby</div>
                    </div>
                </div>
                    ...
                    <div className="mt-12">
                        <form onSubmit={submitForm} >
                            <div>
                                ...
                                <input name="email" type="email" required value={state.userInfo.email} onChange={onChangeValue} className="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" placeholder="mike@gmail.com" />
                            </div>
                            <div className="mt-8">
                                ...
                                <input name="password" type="password" required value={state.userInfo.password} onChange={onChangeValue} className="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" placeholder="Enter your password" />
                            </div>
                            <div className="mt-10">
